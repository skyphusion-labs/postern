...
<div id="app">
  <div class="sidebar">
    <div class="toolbar">
      ...
      <select id="searchMode" class="mode" title="Search mode (applies when searching)">
...
(function () {
  ...
  function apiWrite(path, body) {
    ...
    return fetch(url, {
      ...
      return r.json().catch(function () { return { ok: false, error: "bad_json", status: r.status }; })
        .then(function (j) {
          if (r.status === 401) { var e = new Error("unauthorized"); e.code = 401; throw e; }
          if (r.status === 403) throw new Error((j && (j.message || j.error)) || "requires send scope");
          if (!r.ok || j.ok === false) {
  ...
  function updateComposeButton() {
    $("composeBtn").style.display = state.sendToken ? "" : "none";
  }
  ...
  function logout() {
    ...
    state.token = ""; state.sendToken = ""; state.items = []; state.selected = null; state.cursor = null;
