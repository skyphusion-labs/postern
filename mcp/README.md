...
|---|---|---|
| `mailbox_search` | Search subject + body, newest-first. `mode` defaults to `hybrid` (semantic + keyword); other modes: `fts`, `semantic`, `substr` (literal substring; pair with `field` = `subject`/`body`/`text`). Optional `direction` (`inbound`/`outbound`, the stored fact), `to` / `from`, `lens` (`inbox`/`sent`, one address's view, needs `to`), `mailbox` (`archive`/`trash`/`junk`/`all`, durable folder), `after` / `before` (inclusive ISO date bounds), `hasAttachment`, `seen` (read state), `limit`, `cursor`. `mode=fts` requires every word of the query, so an empty result really means not-here. **The primary tool.** | `GET /api/search` |
