...
| `mailbox_reply` | Reply to a stored message by `message_id` (provide `text` and/or `html`). The server fills `to` / `subject` / `In-Reply-To` / `References` / thread, so the reply lands in the same conversation. Optional `cc`, `bcc`, `from`. **Attachments not yet exposed here:** the worker `/api/reply` has carried `attachments`/`mode`/`quoteOriginal` since #363, but this tool schema does not forward them yet (tracked in #415); attach on `mailbox_send` instead until it lands. | `POST /api/reply` |
