...
```bash
# Provoke exactly one cold sync: LOGIN + SELECT INBOX only (no SEARCH, no body
# open, so nothing else in the catalogue fires). Any account the door authenticates
# proves the wiring; the password is prompted, never argv'd or env'd.
python3 - <<'EOF'
import getpass, imaplib
host = input("door host: ")
user = input("user: ")
m = imaplib.IMAP4_SSL(host, 993)
m.login(user, getpass.getpass("password (not echoed): "))
m.select("INBOX")   # <- this is the cold sync
m.logout()
EOF
